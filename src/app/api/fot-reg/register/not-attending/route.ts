...
const NOT_ATTENDING_LOCATION_ID = 5;
...
export async function POST(request: NextRequest) {
  ...
  const { data: locationData, error: locationErr } = await supabase
    .from("fotlocation")
    .select("id")
    .eq("id", NOT_ATTENDING_LOCATION_ID)
    .limit(1);
  if (locationErr) {
    return NextResponse.json({ error: `Failed to validate not-attending location: ${locationErr.message}` }, { status: 500 });
  }
  if (!Array.isArray(locationData) || locationData.length === 0) {
    return NextResponse.json(
      { error: `Missing not-attending location record (fotlocation.id=${NOT_ATTENDING_LOCATION_ID}).` },
      { status: 500 },
    );
  }
  ...
  const { data: insertedRegData, error: insertRegErr } = await supabase
    ...
    .insert({
      memberid: memberId,
      locationid: NOT_ATTENDING_LOCATION_ID,
